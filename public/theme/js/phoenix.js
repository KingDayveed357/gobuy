(function (global, factory) {
    ...
    function initMap() {
        ...
        if (l.length && window.google) {
            ...
            l.forEach((l) => {
                ...
                (T.appendChild(b),
                    T.appendChild(u),
                    f.controls[window.google.maps.ControlPosition.LEFT].push(
                        T,
                    ));
        ...
        e &&
            e.addEventListener("click", (e) => {
                ...
                "INPUT" === n.tagName &&
                    ...
                    document.execCommand("copy"),
                    (t.querySelector(".toast-body").innerHTML =
                        `<span class="fw-black">Copied:</span>  <code>${n.value}</code>`),
                    o.show());
            });
    };
            ...
            if (window.List) {
                ...
                t.length &&
                    t.forEach((t) => {
                        ...
                        if (a) {
                            (window.phoenix.BulkSelect.getInstance(
                                a,
                            ).attachRowNodes(
                                g.items.map((e) =>
                                    e.elm.querySelector(
                            ...
                            g.on("updated", (e) => {
                                ((m = g.matchingItems.length
                                    ...
                                    y &&
                                        ...
                                        v(),
                                        c &&
                                            (c.innerHTML = `${0 === g.matchingItems.length ? 0 : g.i} to ${0 === g.matchingItems.length ? 0 : b} <span class='text-body-tertiary'> Items of </span>${g.matchingItems.length}`)));
                                const a =
        ...
        (initialDomSetup(n.node),
            n.on("click", (e) => {
                ...
                if (d.data("theme-control")) {
                    ...
                    let h =
                        e.target[
                            "checkbox" === e.target.type ? "checked" : "value"
                        ];
                    ("phoenixTheme" === l &&
                        "boolean" == typeof h &&
                        (h = h ? "dark" : "light"),
                        config.hasOwnProperty(l) &&
                            window.config.set({ [l]: h }));
                    switch (
                        ("true" ===
    ...
    const mapboxInit = () => {
            ...
            o &&
                o.forEach((o) => {
                  window.mapboxgl.accessToken = window.config.mapboxToken;
    ...
    const playOnHoverInit = () => {
        ...
        document
            .querySelectorAll("[data-play-on-container-hover]")
            .forEach((t) => {
