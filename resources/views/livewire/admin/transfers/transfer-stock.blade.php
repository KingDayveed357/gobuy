        ...
        <div class="col-12 col-lg-7">
            <x-admin.card title="Move stock">
                ...
                <div class="position-relative mb-3">
                    <input type="text" class="form-control" wire:model.live.debounce.250ms="search" placeholder="Search products to move…" autocomplete="off">
                    @if (! empty($this->results) && count($this->results))
                        <div class="list-group position-absolute w-100 shadow-sm" style="z-index:30; max-height:280px; overflow:auto;">
                            @foreach ($this->results as $variant)
                                <button type="button" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center py-2" wire:click="addVariant({{ $variant->id }})" wire:key="tr-res-{{ $variant->id }}">
