                                        ...
                                        <tr class="cart-table-row">
                                            ...
                                            <td class="quantity align-middle">
                                                <form action="{{ route('cart.items.update', $item) }}" method="POST" class="d-flex justify-content-center gap-1">
                                                    ...
                                                    <input class="form-control form-control-sm text-center" type="number" name="quantity"
                                                           value="{{ $item->quantity }}" min="0" max="{{ $variant->stock }}" style="width:70px;">
                                                    <button class="btn btn-sm btn-phoenix-secondary" type="submit" title="Update"><span class="fas fa-rotate-right"></span></button>
                                                </form>
                                            </td>
                                            ...
                                            <td class="align-middle text-end pe-0">
                                                <form action="{{ route('cart.items.destroy', $item) }}" method="POST">
                                                    @csrf @method('DELETE')
                                                    <button class="btn btn-sm text-body-tertiary text-danger-hover" type="submit" title="Remove"><span class="fas fa-trash"></span></button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                    ...
                    <div class="col-12 col-lg-4">
                        <div class="card">
                            <div class="card-body">
                                ...
                                @else
                                    <form action="{{ route('cart.coupon.apply') }}" method="POST" class="mt-3 mb-2">
                                        @csrf
                                        <label for="promo-code" class="form-label fs-9 fw-semibold text-body-tertiary mb-1">Have a promo code?</label>
                                        <div class="input-group input-group-sm">
                                            <input type="text" id="promo-code" name="code" class="form-control text-uppercase" placeholder="Enter code" value="{{ old('code') }}" autocomplete="off" required>
                                            <button type="submit" class="btn btn-phoenix-primary">Apply</button>
                                        </div>
                                    </form>
                                @endif
