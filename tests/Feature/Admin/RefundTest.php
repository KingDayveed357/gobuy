...
class RefundTest extends TestCase
{
    ...
    private function paidOrderWithItem(int $stock = 8, int $qty = 2): Order
    {
        ...
        $order->items()->create(['product_id' => $product->id, 'name' => $product->name, 'sku' => $product->sku, 'unit_price' => 1000, 'quantity' => $qty, 'line_total' => 1000 * $qty]);
    ...
    public function test_admin_can_refund_a_paid_order_and_stock_is_restored(): void
    {
        ...
        $this->assertSame(10, Product::find($productId)->stock); // 8 + 2 restocked
    }
